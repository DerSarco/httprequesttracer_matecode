...
let prepareCertificateResult = {
  certLocalPath: "/tmp/httptracer-ca.pem",
  certEmulatorPath: "/sdcard/Download/httptracer-ca.pem",
...
describe("App additional coverage", () => {
  ...
  it("applies interception rules and forwards edited pending requests", async () => {
    ...
    fireEvent.change(screen.getByLabelText("URL"), { target: { value: "https://auth.example.com/session" } });
    fireEvent.change(screen.getByLabelText("Query string"), { target: { value: "via=editor" } });
    fireEvent.change(screen.getByLabelText("Cookies"), { target: { value: "session=updated" } });
    fireEvent.change(screen.getByLabelText("Headers (one per line: Name: Value)"), { target: { value: "X-Test: one\nBroken" } });
