...
describe("App", () => {
  ...
  it("renders detail tabs, masks sensitive values, and exports content", async () => {
    ...
    expect(await screen.findByText("Estado actualizado.")).toBeInTheDocument();
    expect(await screen.findByTitle("https://example.com/api?foo=bar&baz=qux")).toBeInTheDocument();
  ...
  it("manages interception rules and forwards pending requests", async () => {
    ...
    fireEvent.change(urlInput, { target: { value: "https://api.example.com/v1/login?debug=false" } });
    fireEvent.change(queryInput, { target: { value: "debug=false" } });
    fireEvent.change(cookiesInput, { target: { value: "session=changed" } });
    fireEvent.change(headersInput, { target: { value: "Content-Type: application/json\nX-Test: 1" } });
